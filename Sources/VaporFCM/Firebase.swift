...
public class Firebase {
    ...
    public func send(message: Message, toMultiple targets: [Targetable]) throws -> FirebaseResponse {
        let requestBytes: Bytes = try serializer.serialize(message: message, targets: targets)
        do {
            let response = try requestAdapter.send(bytes: requestBytes, headers: generateHeaders(), url: pushUrl)

            if let json = try? Jay().anyJsonFromData(response.body.bytes ?? []),
